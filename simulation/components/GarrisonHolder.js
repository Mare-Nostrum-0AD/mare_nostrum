			...
			"<zeroOrMore>" +
				"<element a:help='Element containing the offset coordinates'>" +
					...
					"<interleave>" +
						...
						"</optional>" +
						"<optional>" +
							"<element name='List' a:help='Classes of entities which are allowed to garrison at this garrison point (from Identity)'>" +
								"<attribute name='datatype'>" +
									"<value>tokens</value>" +
								"</attribute>" +
								"<text/>" +
	...
	if (this.template.VisibleGarrisonPoints)
	{
		...
		for (let point in points)
			this.visibleGarrisonPoints.push({
				...
				"list": points[point].List ? points[point].List : null,
				"entity": null
			});
	}
};
...
GarrisonHolder.prototype.Garrison = function(entity, vgpEntity)
{
	...
	if (!visibleGarrisonPoint)
		for (let vgp of this.visibleGarrisonPoints)
		{
			if (vgp.entity)
				continue;
			if (vgp.list != null && !MatchesClassList(entityClasses, vgp.list._string))
				continue;
	...
	if (visibleGarrisonPoint)
	{
		...
		// ensure packable units stay unpacked
		let cmpPack = Engine.QueryInterface(entity, IID_Pack);
		if (cmpPack)
		{
			cmpPack.SetUnpacked();
			cmpPack.LockState();
...
GarrisonHolder.prototype.Eject = function(entity, forced)
{
	...
	let cmpEntPack = Engine.QueryInterface(entity, IID_Pack);
	if (cmpEntPack)
	{
		cmpEntPack.SetPacked();
		cmpEntPack.UnlockState();
	}
