...
Upgrade.prototype.Init = function()
{
	...
	for (let choice in this.template)
	{
		...
		if (cmpIdentity)
			name = parseCivTemplate(name, /\{civ\}/g, cmpIdentity.GetCiv());
...
Upgrade.prototype.GetUpgrades = function()
{
	...
	for (let option in this.template)
	{
		...
		let templateName = cmpIdentity ? parseCivTemplate(choice.Entity, /\{civ\}/g, cmpIdentity.GetCiv()) : choice.Entity;
...
Upgrade.prototype.GetRequiredTechnology = function(templateArg)
{
	...
	if (cmpIdentity)
		entType = parseCivTemplate(entType, /\{civ\}/g, cmpIdentity.GetCiv());
