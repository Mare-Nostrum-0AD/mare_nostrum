 ...
 */
Resources.prototype.GetCodes = function(filter = (res) => true)
{
	return this.resourceData.filter(filter).map((data) => data.code);
};
 ...
 */
Resources.prototype.GetNames = function(filter = (res) => true)
{
	let output = {};
	this.resourceData.filter(filter).forEach((data) => { output[data.code] = data.name });
	return output;
};
