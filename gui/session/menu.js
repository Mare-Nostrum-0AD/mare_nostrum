...
function resizeDiplomacyDialog()
{
	...
	let widthOffset = g_ResourceData.GetCodes((res) => res.shareable === true).length * (tribSize.right - tribSize.left) / 2;
...
{
	let resCodes = g_ResourceData.GetCodes((res) => res.shareable === true);
	let r = 0;
...
function resizeTradeDialog()
{
	...
	width += g_ResourceData.GetCodes((res) => res.tradeable).length * (tradeSize.right - tradeSize.left);
...
function openTrade()
{
	...
	let button = {};
	let resCodes = g_ResourceData.GetCodes((res) => res.tradeable);
...
function updateBarterButtons()
{
	...
	if (canBarter)
		g_ResourceData.GetCodes((res) => res.tradeable).forEach((resCode, i) => { barterUpdateCommon(resCode, i, "barter", g_ViewedPlayer) });
}
