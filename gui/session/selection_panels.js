...
g_SelectionPanels.Barter = {
	...
	"getItems": function(unitEntStates)
	{
		...
		if (unitEntStates.every(state => !state.isBarterMarket) || g_ResourceData.GetCodes((res) => res.tradeable).length > this.rowLength)
			return [];
		return g_ResourceData.GetCodes((res) => res.tradeable);
	},
	"setupButton": function(data)
	{
