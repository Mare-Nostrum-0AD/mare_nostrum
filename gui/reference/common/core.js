...
function loadBuildQueue(template)
{
	...
	{
		if (build.indexOf('{civ}') != -1)
			build = parseCivTemplate(build, /\{civ\}/g, g_SelectedCiv);
		else if (build.indexOf('{native}') != -1)
			build = parseCivTemplate(build, /\{native\}/g, g_SelectedCiv);
		if (build && Engine.TemplateExists(build))
