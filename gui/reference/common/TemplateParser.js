...
class TemplateParser
{
	...
	getEntity(templateName, civCode)
	{
		...
		let templateClasses = template.Identity.VisibleClasses ? template.Identity.VisibleClasses._string.split(/\s+/g) : [];
		if (template.Identity.Classes)
			templateClasses.push(...template.Identity.Classes._string.split(/\s+/g));
		...
		if (templateClasses.indexOf('CivCentre') !== -1)
		{
			parsed.isCivCentre = true;
			for (let phase of this.phaseList.slice().reverse())
			{
				let phaseClass = "Phase" + phase[6].toUpperCase() + phase.substr(7);
				if (templateClasses.indexOf(phaseClass) !== -1)
				{
					parsed.phase = phase;
					break;
				}
	...
	getActualUpgradeData(upgradesInfo, civCode)
	{
		...
		{
			upgrade.entity = parseCivTemplate(upgrade.entity, /\{civ\}/g, civCode);
			if (!upgrade.entity)
				continue;
