...
class TemplateLoader
{
	...
	deriveProductionQueue(template, civCode)
	{
		...
		if (template.ProductionQueue.Entities && template.ProductionQueue.Entities._string)
			...
			{
				templateName = templateName.indexOf('{native}') !== -1 ?
					parseCivTemplate(templateName, /\{native\}/g, template.Identity.Civ) :
					parseCivTemplate(templateName, /\{civ\}/g, civCode);
				if (templateName)
	...
	deriveBuildQueue(template, civCode)
	{
		...
		{
			build = build.indexOf('{native}') !== -1 ?
				parseCivTemplate(build, /\{native\}/g, template.Identity.Civ) :
				parseCivTemplate(build, /\{civ\}/g, civCode);
			if (build)
