...
function updateCountersPlayer(playerState, allCounters, allHeadings, idGUI, index)
{
	...
	for (let n in counters)
	{
		...
		Engine.GetGUIObjectByName(sprintf('%s[%d]', idGUI, n)).caption =
...
function calculateTeamCounterDataHelper()
{
	for (let i = 0; i < g_PlayerCount; ++i)
	{
		...
		g_TeamHelperData[playerState.team].civicPopulation = playerState.sequences.teamPercentMapExplored;
	}
}
...
function calculateCivicPopulation(playerState, index)
{
	return playerState.sequences.civicPopulation[index];
}

function calculateMiscellaneousTeam(team, index, type, counters, headings)
{
	if (type == "civicPopulation")
		return g_TeamHelperData[team].civicPopulation[index];
