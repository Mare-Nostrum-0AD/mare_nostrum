...
function getCityPopulationText(template)
{
	...
	if (template.city.population.growth.tradeRate)
		tooltip += sprintf(" + %(rate)s per unit trade income", { "rate": template.city.population.growth.tradeRate });
...
function getCityUpgradeText(template, playerCiv)
{
	...
	let text = upgrade.Identity.SpecificName ?
		"Upgrades to %(specificName)s (%(genericName)s) at %(maxPop)s" :
		"Upgrades to %(genericName)s at %(maxPop)s";
